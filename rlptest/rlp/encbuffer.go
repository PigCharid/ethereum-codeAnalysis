...
type encBuffer struct {
	str     []byte     // string data, contains everything except list headers  // 包含了除了列表的头部的所有的编码的内容
	lheads  []listhead // all list headers  //所有的列表头
	lhsize  int        // sum of sizes of all encoded list headers  //lheads的长度
	sizebuf [9]byte    // auxiliary buffer for uint encoding 	//9个字节大小的辅助buffer，用来处理uint的编码
}
